...
<body
            ...
            <div class="card bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
                ...
                <script>
                    function filterPurchases() {
                        ...
                        purchaseSelect.innerHTML = '<option value="" disabled selected>Select Purchase</option>';

                        // Filter purchases based on selected buyer
                        const purchases = Array.from(purchaseSelect.querySelectorAll('option[data-buyer-id]'));
                        purchases.forEach(option => {
                            if (option.getAttribute('data-buyer-id') === selectedBuyerId) {
            ...
            <form id="addItemForm" class="space-y-4 grid grid-cols-2 gap-4" method="POST"
                ...
                <div class="flex flex-row gap-2 items-end">
                    ...
                    <button type="button"
                        ...
                        onclick="openAddGradeModal()" title="Add Grade">
                        <i class="fa-solid fa-plus"></i>
                    </button>
                </div>
                ...
                <div class="flex flex-row gap-2 items-end">
                    ...
                    <button type="button"
                        class="btn-primary h-10 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium flex items-center justify-center transition-colors duration-200"
                        onclick="openAddFinishingModal()" title="Add Finishing">
                        <i class="fa-solid fa-plus"></i>
                    </button>
                </div>
                <div>
    ...
    <div id="addFinishingModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 hidden">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-md p-6 relative">
            <button onclick="closeAddFinishingModal()"
                class="absolute top-3 right-3 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors duration-200">
                <i class="fa-solid fa-x"></i>
            </button>
            ...
            <form id="addFinishingForm" class="space-y-4" method="POST"
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" onclick="closeAddFinishingModal()"
    ...
    <div id="addGradeModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 hidden">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-md p-6 relative">
            <button onclick="closeAddGradeModal()"
                class="absolute top-3 right-3 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors duration-200">
                <i class="fa-solid fa-x"></i>
            </button>
            <h3 class="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Add New Grade</h3>
            <form id="addGradeForm" class="space-y-0 flex items-center gap-2 w-full" method="POST" action="{{ route('barcode.grades.store') }}">
                @csrf
                <div class="flex-1">
    ...
    <script>
        ...
        function openAddFinishingModal() {
            document.getElementById('addFinishingModal').classList.remove('hidden');
            document.body.classList.add('overflow-hidden');
        }

        function closeAddFinishingModal() {
            document.getElementById('addFinishingModal').classList.add('hidden');
            document.body.classList.remove('overflow-hidden');
        }
        ...
        function deleteFinishing(id) {
            ...
            document.getElementById('confirmDeleteFinishing').onclick = function() {
                ...
                .then(data => {
                    // Remove from select dropdown
                    const finishingSelect = document.getElementById('finishing_id');
                    const optionToRemove = Array.from(finishingSelect.options).find(option => option.value == id);
        ...
        // Filter Warna Anyam based on Jenis Anyam selection
        function filterWarnaAnyam() {
            const jenisAnyamId = document.getElementById('jenisanyam_id').value;
            const warnaAnyamSelect = document.getElementById('warnaanyam_id');
            const options = warnaAnyamSelect.querySelectorAll('option');

            // Reset and disable the select
            warnaAnyamSelect.value = '';
            warnaAnyamSelect.disabled = !jenisAnyamId;
            ...
            options.forEach(option => {
                ...
                const matchesJenisAnyam = option.dataset.jenisanyamId === jenisAnyamId;
                option.style.display = matchesJenisAnyam ? '' : 'none';
                option.disabled = !matchesJenisAnyam;
            });
        }

        // Initialize the filter on page load if needed
        document.addEventListener('DOMContentLoaded', function() {
            const jenisAnyamSelect = document.getElementById('jenisanyam_id');
            if (jenisAnyamSelect) {
                jenisAnyamSelect.addEventListener('change', filterWarnaAnyam);
                // Trigger change event if there's a selected value
                if (jenisAnyamSelect.value) {
                    filterWarnaAnyam();
                }
            }
        });
        ...
        function filterWarnaAnyam() {
            const jenisanyamSelect = document.getElementById('jenisanyam_id');
            const warnaanyamSelect = document.getElementById('warnaanyam_id');
            const selectedJenisanyamId = jenisanyamSelect.value;

            // Save all options on first run
            if (!window._allWarnaAnyamOptions) {
                window._allWarnaAnyamOptions = Array.from(warnaanyamSelect.options);
            }

            // Clear options
            warnaanyamSelect.innerHTML = '<option value="" disabled selected>Select Warna Anyam</option>';

            // Add filtered options
            window._allWarnaAnyamOptions.forEach(option => {
                if (!option.dataset.jenisanyamId || option.dataset.jenisanyamId === selectedJenisanyamId) {
                    warnaanyamSelect.appendChild(option.cloneNode(true));
                }
            });
        };

        function updateCodeOptions() {
            const originSelect = document.getElementById('originLineSelect');
            const codeSelect = document.getElementById('codeSelect');
            const selectedOption = originSelect.options[originSelect.selectedIndex];
            const codes = selectedOption.getAttribute('data-codes');
